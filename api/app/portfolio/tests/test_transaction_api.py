...
class PrivateApiTests(TestCase):
	...
	def test_buying_stock(self):
		...
		self.assertEqual(
			portfolio.balance,
			initial_balance - payload['price_per_share']*payload['number_of_shares']
		)
		...
		self.assertEqual(str(stock), str(transaction.stock)) #not sure about this yet 
	...
	def test_selling_all_stock(self):
		...
		payload = {'is_buy':False, 'price_per_share':200.00, 'number_of_shares':2, 'stock_id':stock.id}
		res = self.client.post(transaction_url(portfolio.id), payload)
		portfolio.refresh_from_db()
		self.assertEqual(res.status_code, status.HTTP_201_CREATED)
		self.assertEqual(
