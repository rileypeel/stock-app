...
class TransactionView(APIView):
	...
	def post(self, request, id):
		...
		try:
			holding = Holding.objects.filter(portfolio=portfolio).get(stock=stock)
		except Holding.DoesNotExist:
			holding = None

		context = {'portfolio':portfolio, 'stock':stock, 'holding':holding}
		serializer = serializers.TransactionSerializer(data=request.data, context=context)
		...
		if serializer.is_valid():
			...
			if request.data['is_buy']=='True':
				if holding is None:
					...
					holding.number_of_shares=int(holding.number_of_shares)+num_shares
					holding.save()
