...
class DailyData(TestCase):
    ...
    def test_adding_stock(self, gd):
        ...
        for dp in daily_data:
            self.assertEqual(str_to_decimal(
                data[datetime_to_str(dp.time_stamp)]['1. open']), dp.open_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str(dp.time_stamp)]['4. close']), dp.close_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str(dp.time_stamp)]['3. low']), dp.low_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str(dp.time_stamp)]['2. high']), dp.high_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str(dp.time_stamp)]['6. volume']), dp.volume)
    ...
    def test_violate_unique_together(self, gd):
        ...
        stock = Stock.objects.get(ticker='TSLA')
        timestamp = DailyPrice.objects.filter(
            stock=stock).latest('time_stamp').time_stamp
        unique_error = False
        try:
    ...
    def test_update_all(self, ts):
        ...
        msft_last_price = DailyPrice.objects.filter(
            stock=msft).latest('time_stamp')
        ...
        for key in data.keys():
            try:
                dp = DailyPrice.objects.get(
                    stock=tsla, time_stamp=str_to_datetime(key))
                self.assertEqual(str_to_decimal(
                    data[key]['1. open']), dp.open_price)
                self.assertEqual(str_to_decimal(
                    data[key]['4. close']), dp.close_price)
                self.assertEqual(str_to_decimal(
                    data[key]['3. low']), dp.low_price)
                self.assertEqual(str_to_decimal(
                    data[key]['2. high']), dp.high_price)
        ...
        for key in data.keys():
            ...
            try:
                dp = DailyPrice.objects.get(
                    stock=msft, time_stamp=str_to_datetime(key))
                self.assertEqual(str_to_decimal(
                    data[key]['1. open']), dp.open_price)
                self.assertEqual(str_to_decimal(
                    data[key]['4. close']), dp.close_price)
    ...
    def test_updating_last_day(self):
        ...
        for dp in tsla_prices:
            self.assertIn(datetime_to_str(dp.time_stamp),
                          tsla_update_data1['Time Series (Daily)'].keys())
...
class IntradayData(TestCase):
    ...
    def test_adding_intraday_stock(self, gi):
        ...
        for ip in intra_data:
            self.assertEqual(str_to_decimal(
                data[datetime_to_str_minute(ip.time_stamp)]['1. open']), ip.open_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str_minute(ip.time_stamp)]['4. close']), ip.close_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str_minute(ip.time_stamp)]['3. low']), ip.low_price)
            self.assertEqual(str_to_decimal(
                data[datetime_to_str_minute(ip.time_stamp)]['2. high']), ip.high_price)
            self.assertEqual(int(data[datetime_to_str_minute(
                ip.time_stamp)]['5. volume']), int(ip.volume))
