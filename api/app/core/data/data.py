...
def get_intraday(ticker, all_data, interval):
	...
	if not all_data:
		data_size = 'compact'
	try:
		res = get(f"""{INTRADAY_TIME_SERIES_URL}&outputsize={data_size}&symbol={ticker}&interval={interval}&apikey={ALPHA_VANTAGE_KEY}&datatype=json""")
...
def get_daily(ticker, all_data):	
	...
	try:
		res = get(f"{TIME_SERIES_URL}&symbol={ticker}&outputsize={data_size}&apikey={ALPHA_VANTAGE_KEY}&datatype=json")
	except requests.exception.RequestExceptions:
...
def update_prices():
	...
	for s in stocks:
		...
		if s.ticker in intraday_stocks:
			latest_intra_price = MinutePrice.objects.filter(stock=s).latest('time_stamp')
...
def update_stock(stock):
	...
	if stock.ticker in intraday_stocks:
		latest_intra_price = MinutePrice.objects.filter(stock=stock).latest('time_stamp')
